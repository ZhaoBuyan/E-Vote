...
$('#createPollForm').on('submit', function (e) {
    ...
    var maxChoices = parseInt($('#maxChoices').val()) || 0
    if (!title) {
...
function fillFormDirectly(title, description, options) {
    ...
    $('#batchStatus')
        .text('✅ 已直接填充到表单（' + options.length + ' 个选项）')
        .css('color', '#28a745')
}
        ...
        success: function (res) {
            if (res.code === 200) {
                ...
                if (userOptions && userOptions.length > 0) {
                    ...
                    console.log(
                        '✅ 批量填充：使用用户提供的选项（共 ' + userOptions.length + ' 个）'
                    )
                    ...
                    console.log(
                        '✅ 批量填充：使用 AI 生成的选项（共 ' + data.options.length + ' 个）'
                    )
                } else {
                    $('#optionsContainer').empty()
                    addOption('')
                ...
                if (data._mock) {
                    $status
                        .text('⚠️ AI 服务不可用，已直接填充（请手动调整）')
                        .css('color', '#856404')
                } else {
                    $status
                        .text('✅ AI 优化完成！表单已更新（保留用户选项）')
                        .css('color', '#28a745')
                }
            } else {
...
function batchFill(useAI) {
    ...
    if (useAI) {
        ...
        if (isContentComplete(title, description, options)) {
            ...
            $status
                .text('✅ 内容完整，已直接填充（' + options.length + ' 个选项）')
                .css('color', '#28a745')
        } else {
